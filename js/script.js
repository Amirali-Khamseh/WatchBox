...
async function fetchData(endPoint) {
  const result = await fetch(
    `${state.api.APIURL}${endPoint}?api_key=${state.api.APIKey}&language=en-US`
  );
  showSpinner();
  data = result.json();
  hideSpinner();
  ...
  results.forEach((movie) => {
    ...
    div.innerHTML = `    
        ...
        <div class="card-body">
          ...
          <p class="card-text">
            <small class="text-muted">⭐${movie.vote_average.toFixed(
              1
            )}/10</small>
          </p>
        </div>
...
async function searchAPI() {
  const result = await fetch(
    `${state.api.APIURL}search/${state.searchObj.type}?api_key=${state.api.APIKey}&language=en-US&query=${state.searchObj.term}&page=${state.searchObj.page}`
  );
  showSpinner();
  data = result.json();
  hideSpinner();
